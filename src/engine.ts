/** devcontainer-forge — engine by zAx4hub (algo flavor: jaccard) */
...
export function rank(text: string, seed = 32): number {
  ...
  if ("jaccard" === "topk") score = Math.min(1, score * 1.05);
  if ("jaccard" === "bloom") score = Math.min(1, score * 0.98 + 0.02);
  return Math.round(score * 1000) / 1000;
}

export function run(input: { items?: Item[]; threshold?: number; baseline?: string; seed?: number } = {}): Report {
  const items = input.items?.length ? input.items : [{ text: "devcontainer-forge" }];
  const threshold = input.threshold ?? 0.35;
  const seed = input.seed ?? 32;
  ...
  return {
    project: "devcontainer-forge",
    author: "zAx4hub",
    algo: "jaccard",
    summary: `Processed ${findings.length} items for devcontainer-forge; avg=${avg.toFixed(3)}`,
...
export function demo(): Report {
  return run({
    items: [
      { text: "Generate hardened devcontainers from deps" },
...
export function inspect() {
  return {
    ...
    author: "zAx4hub",
    oneLiner: "Generate hardened devcontainers from deps",
    algo: "jaccard",
    version: "0.1.0",
